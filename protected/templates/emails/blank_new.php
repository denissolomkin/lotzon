<!DOCTYPE HTML>
<html>

<head>
    <meta charset="utf-8">
    <title>%header%</title>
</head>
<style type="text/css">
  a{
      color: #4a90e2;
      text-decoration: none;
  }
</style>
<body style="margin:0 0 0 0;background-color:#f7f7f7">
    <table style="background-color:#f7f7f7;width:100%;height:100%">
        <tr>
            <td valign="top">
                <table style="width:90%; max-width:650px;margin:auto;border-collapse:collapse;">
                    <tr>
                        <td>
                            <div style="width:100%;padding: 30px 0;">
                                <table width="100%" align="center" bgcolor="#ffffff" border="0" cellpadding="1" data-mobile="true" style="border-collapse:collapse;">
                                    <tr>
                                        <td bgcolor="#ffe401" height="80px" width="100%">
                                            <table style="width:100%;height:100%;border-collapse:collapse;">
                                                <tr>
                                                    <td style="height: 100%; width: 30px;">
                                                    </td style>
                                                    <td style="">
                                                        <a href="https://lotzon.com/?utm_source=direct&utm_medium=email&utm_content=logo&utm_campaign=%taskId%" style="display:block;">
                                                            <img style="" src="http://lotzon.com/tpl/img/email_logo_new.png">
                                                        </a>
                                                    </td>
                                                </tr>
                                            </table>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td height="25px" width="100%"></td>
                                    </tr>
                                    <tr>
                                        <td style="font-family:Arial;font-size:20px;color:#000000;padding: 0 20px 20px 20px;line-height:20px">
                                            %title%
                                        </td>
                                    </tr>
                                    <tr>
                                        <td style="font-family:Arial;font-size:15px;color:#4a4a4a;padding: 0 20px 60px 20px;line-height:20px">
                                            %body_text%
                                        </td>
                                    </tr>
                                    <tr>
                                        <td style="font-family:Arial;font-size:15px;color:#4a4a4a;padding: 0 20px 0 20px;line-height:20px">
                                            %footer%
                                        </td>
                                    </tr>
                                    <tr>
                                        <td height="25px" width="100%"></td>
                                    </tr>
                                </table>
                            </div>
                            <div style="width:100%;">
                                <table style="width:100%;">
                                    <tr>
                                        <td style="width:20px"></td>
                                        <td style="font-family:Arial;font-size:15px;color:#4a4a4a;">%footer_social%</td>
                                        
                                        <td style="">
                                            <a href="https://www.facebook.com/pages/Lotzon/714221388659166" style="color: #374c85; text-decoration: none;">Facebook</a>
                                        </td>
                                        <td style="">
                                            <a href="http://vk.com/lotzon" style="color: #2a598f; text-decoration: none;">ВКонтакте</a>
                                        </td>
                                        <td style="">
                                            <a href="http://ok.ru/group/52501162950725" style="color: #ed812b; text-decoration: none;">Одноклассники</a>
                                        </td>
                                        <td style="width: 20%;">
                                        </td>
                                        <td style="width:20px"></td>
                                    </tr>
                                </table>
                            </div>
                            <div style="width:100%;">
                                <table>
                                    <tr>
                                        <td style="width:20px"></td>
                                        <td style="">
                                            <p style="margin:20px 0;line-height:1.2;font-family:Arial;font-size:15px;color:#a7a7a7;">%sub_footer%</p>
                                        </td>
                                        <td style="width:20px"></td>
                                    </tr>
                                </table>
                            </div>
                        </td>
                    </tr>
                </table>
            </td>
        </tr>
    </table>
</body>

</html>
