<!DOCTYPE HTML>
<html>

<head>
  <meta charset="utf-8">
  <title>%header%</title>
</head>

<body style="margin:0 0 0 0;background-color:#f7f7f7">
  <table style="background-color:#f7f7f7;width:100%;height:100%">
    <tr>
      <td valign="top">
        <table style="width:90%; max-width:600px;margin:auto;">
          <tr>
            <td>
              <div style="max-width:600px; border: 1px solid #d4d4d4">
                <table width="100%" align="center" bgcolor="#ffffff" border="0" cellpadding="1" data-mobile="true">
                  <tr>
                    <td bgcolor="#ffe401" height="50px" width="100%">
                      <table style="width:130px;height:100%">
                        <tr style="width:130px;height:100%">
                          <td style="width:130px;min-width:28px;width:130px;height:100%">
                            <a href="https://lotzon.com/?utm_source=direct&utm_medium=email&utm_content=logo&utm_campaign=%taskId%" style="width:100%; display:block; height:50px;">
                              <img style="width:130px;height:50px" src="http://lotzon.com/tpl/emails/img/logo.png">
                            </a>
                          </td>
                        </tr>
                      </table>
                    </td>
                  </tr>
                  <tr>
                    <td height="30px" width="100%"></td>
                  </tr>
                  <tr>
                    <td style="font-size: 16px; font-family: Arial;font-size:14px; padding-left:20px; padding-right:20px; line-height:20px">
                      %body_up_text%
                    </td>
                  </tr>
                  <tr>
                    <td height="30px" width="100%"></td>
                  </tr>
                  <tr width="100%">
                    <td>
                      <table style="padding-left:18px;padding-right:20px">
                        <tr>
                          <td width=100% style="color:#fff;width:350px;height:30px;text-align:center" bgcolor="#4990e2">
                            <a style="font-family: Arial;font-size:14px; display:block; text-decoration: none; color: #fff; padding:5px" href="#" target="_blank">%body_confirm_button%</a>
                          </td>

                        </tr>
                      </table>
                    </td>
                  </tr>
                  <tr>
                    <td height="5px" width="100%"></td>
                  </tr>
                  <tr>
                    <td style="padding-left:20px">
                      <p style="line-height:20px;font-family: Arial;font-size:14px;padding-right:20px">%body_confirm_link_description%</p>
                      <p style="line-height:15px"><a style="color:#4A90E2" href="#" target="_blank">http://********************</a></p>
                    </td>
                  </tr>
                  <tr>
                    <td height="30px" width="100%"></td>
                  </tr>
                  <tr>
                    <td style="padding-left:20px;font-family:Arial;font-size:14px;line-height:20px;padding-right:20px">
                      %body_down_text%
                    </td>
                  </tr>
                  <tr>
                    <td height="30px" width="100%"></td>
                  </tr>
                  <tr>
                    <td style="background-color:#fafafa">
                      <table style="width:100%;height:50px;background-color:#fafafa">
                        <tr>
                          <td style="width:20px"></td>
                          <td style="font-family:Arial;font-size:13px">%footer_social%</td>
                          <td style="width:28px;min-width:28px">
                            <a href="https://www.facebook.com/pages/Lotzon/714221388659166" style="width:100%;display:block;height:100%"><img style="width:28px;height:50px" src="http://lotzon.com/tpl/emails/img/fb.png"></a>
                          </td>
                          <td style="width:28px;min-width:28px">
                            <a href="http://vk.com/lotzon" style="width:100%; display:block; height:100%"><img style="width:28px;height:50px" src="http://lotzon.com/tpl/emails/img/vk.png"></a>
                          </td>
                          <td style="width:28px;min-width:28px">
                            <a href="https://twitter.com/LOTZON_COM" style="width:100%; display:block; height:100%"><img style="width:28px;height:50px" src="http://lotzon.com/tpl/emails/img/tw.png"></a>
                          </td>
                          <td style="width:28px;min-width:28px">
                            <a href="https://plus.google.com/112273863200721967076/about" style="width:100%; display:block; height:100%"><img style="width:28px;height:50px" src="http://lotzon.com/tpl/emails/img/gplus.png"></a>
                          </td>
                          <td style="width:35%"></td>
                          <td style="font-family:Arial;font-size:13px">%footer_site% <a href="https://lotzon.com/?utm_source=direct&utm_medium=email&utm_content=footer_site&utm_campaign=%taskId%" target="_blank" style="text-decoration: none">lotzon.com</a></td>
                          <td style="width:20px"></td>
                        </tr>
                      </table>
                    </td>
                  </tr>
                </table>
              </div>
              <div style="max-width:600px">
                <table>
                  <tr>
                    <td style="width:100%">
                      <p style="padding-left:20px;padding-right:20px;line-height:13px;font-family: Arial;font-size:11px;color:#9b9b9b;margin-top: 20px">%sub_footer%</p>
                    </td>
                  </tr>
                </table>
              </div>
            </td>
          </tr>
        </table>
      </td>
    </tr>
  </table>
</body>

</html>