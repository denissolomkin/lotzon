                ...
                if (index == Player.id) {
                    ...
                    $('.mx .players .player' + index + ' .gm-pr')
                        ...
                        .append(
                            ...
                            '<div class="pr-pr"><div class="title">' + i18n('title-games-bet-once') + '</div><div><b>' + (bet[0] == 'MONEY' ? Player.formatCurrency(bet[1], 1) : bet[1]) + '</b><span>' + (bet[0] == 'MONEY' ? Player.formatCurrency(bet[1], 2) : 'баллов') + '</span></div></div>' +
                            '<div class="pr-pt"><div class="title">' + i18n('title-games-balance') + '</div>' +
                            (App.mode.indexOf('MONEY') != -1 ? '<div><span class="holder-money">' + Player.balance.money + '</span> ' + Player.formatCurrency() + '</div>' : '<div><span class="holder-points">' + Player.balance.points + '</span> ' + i18n('title-of-points') + '</div></div>')
                        );
                }
            });
        },
        ...
        do: {
            ...
            ready: function(e) {
                ...
                if ((price[0] == 'POINT' && Player.balance.points < parseInt(price[1])) || (price[0] == 'MONEY' && Player.balance.money < Player.formatCurrency(price[1], 1))) {
                    ...
                    $("#report-popup").show().find(".txt").text(i18n('INSUFFICIENT_FUNDS')).fadeIn(200);
